...
const handleAddNote = () => {
  ...
  if (title.value === '')
    return (titleErrorMessage.value =
      'Bitte einen Titel eingeben!');
  if (text.value === '')
    return (textErrorMessage.value =
...
<template>
  <div class="mb-3">
    ...
    />
    <ErrorMessage v-if="titleErrorMessage">{{
      titleErrorMessage
    }}</ErrorMessage>
  </div>
  <div class="mb-3">
    ...
    />
    <ErrorMessage v-if="textErrorMessage">{{
      textErrorMessage
    }}</ErrorMessage>
  </div>
  <FlexJustifyBetween>
    <Button
      :onClick="
        noteToUpdate ? handleUpdateNote : handleAddNote
      "
    >
